...
// FILE STREAMS
// Create rotating file stream for SQL errors
var sqlErrorStream = rfs.createStream('./sqlError.log', {
    interval: '24h', // rotate every 24 hours
    path: path.join(__dirname, '../log/')
})
// Create rotating file stream for input validation errors
var validationErrorStream = rfs.createStream('./validationError.log', {
    interval: '24h',
    path: path.join(__dirname,'../log/')
})
// Create rotating file stream for verifyAdmin errors
var verifyAdminErrorStream = rfs.createStream('./verifyAdminError.log', {
    interval: '24h',
    path: path.join(__dirname,'../log/')
})

// CUSTOM TOKENS
// Create token for SQL errors
morgan.token('sqlError', function (req, res) {
    if (res.sqlError) {
        return res.sqlError
    }
})
// Create token for validation errors
morgan.token('validationError', function(req, res) {
    if (res.validationError) {
        return res.validationError
    }
})
// Create token for request body during validation errors
...
// LOGGERS
// Set up logger for SQL errors
app.use(morgan(function (tokens, req, res) {
        // Specify the log format
        log = {
            'sqlError': tokens.sqlError(req, res),
    ...
    {
        skip: function (req, res) {return !res.sqlError}, // only log if the sqlError field is present in the res object
        stream: sqlErrorStream // set the stream
    })
);
...
app.use(morgan(function(tokens, req, res) {
        // Specify the log format
        log = {
            'validationError': tokens.validationError(req,res),
            'requestBody': tokens.requestBody(req,res),
            'method': tokens.method(req,res),
            'url': tokens.method(req,res),
            'ip': tokens['remote-addr'](req,res),
            'date':tokens.date(req,res)
        };
        return JSON.stringify(log);
    ...
    {
        skip: function(req, res) {return !res.validationError},
        stream: validationErrorStream
    })
);
// Set up logger for admin verification errors
app.use(morgan(function(tokens, req, res) {
    // Specify the log format
    log = {
        'method': tokens.method(req,res),
...
{
    skip: function(req, res) {return !res.verifyAdminError},
    stream: verifyAdminErrorStream
})
);
...
//Delete Review
app.delete('/review/:reviewid', verifyToken,  validation.validateUserid, validation.validateReviewid, (req, res) => {

    reviewDB.deleteReview(req.params.reviewid, req.userid, (err, results) => {
        if (err) {

            if (err.sqlMessage) {
                res.sqlError = err.sqlMessage;
                res.status(422).send()
            } else {
                res.status(500).json({ result: "Internal Error" })
            }
...
app.post('/users', validation.validateRegister, (req, res) => {

    var { username, email, contact, password, profile_pic_url } = req.body;

    // Bcrypt hash the password before sending to query
...
app.post('/category', verifyToken, verifyAdmin, (req, res) => {
...
app.post('/product', verifyToken, verifyAdmin, (req, res) => {
...
app.post('/product/:id/review/', verifyToken, validation.validateUserid, validation.validateReviewid, validation.validateReview, (req, res) => {

    var userid = req.userid; // this is decoded from the JWT via the verifyToken middleware
...
app.get('/product/:id/reviews', (req, res) => {
    ...
    reviewDB.getProductReview(req.params.id, (err, results) => {
        ...
        //No error, response with all user info
            ...
            // Validate the output using the sanitizeOutput function
            var sanitizedResults = validation.sanitizeOutput(results)
...
app.post('/product/:id/image', verifyToken, verifyAdmin, upload.single('image'), function (req, res) {
